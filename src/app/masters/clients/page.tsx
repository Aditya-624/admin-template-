...
export default function ClientsListPage() {
    ...
    API.get("/api/master/clients")
      .then((res) => {
        if (Array.isArray(res.data) && res.data.length > 0) {
          ...
          setClients(mapped);
          localStorage.setItem(CLIENTS_STORAGE_KEY, JSON.stringify(mapped));
          setError(null);
        }
      })
      .catch(() => {
  ...
  const sorted = React.useMemo(() => {
    ...
    data.sort((a, b) => {
      let aVal = a[sortColumn as keyof Client] ?? "";
      let bVal = b[sortColumn as keyof Client] ?? "";
    ...
    <div className="datatable-page" style={{ display: "flex", justifyContent: "center", padding: "24px" }}>
      ...
      <div className="table-card" style={{ maxWidth: "100%", width: "100%" }}>
        <div className="datatable-toolbar" style={{ justifyContent: "space-between" }}>
          <h1 className="text-2xl font-bold text-white">Clients List</h1>
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              onClick={() => router.push("/masters/clients/add")}
            >
              <span className="btn-label">Add Client</span>
            </button>
          </div>
        </div>
              ...
              {loading ? (
                ...
                sorted.map((c, index) => (
                  <tr key={c.id} className={index % 2 === 0 ? "bg-white/[0.01]" : ""}>
                    ...
                    <td>
                      <div className="datatable-cell">{c.clientName}</div>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="datatable-cell">{c.gstNumber}</div>
                    </td>
                    <td>
                      <div className="datatable-actions justify-center">
                        <button
                          ...
                          title="Edit"
                          onClick={() => router.push(`/masters/clients/${c.id}/edit`)}
                        >
                          <Edit size={16} />
                        </button>
                        <button
