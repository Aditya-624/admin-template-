...
export default function EditMaterialMeta1Page() {
  ...
  // Load existing record and masters
  useEffect(() => {
    // 1. Fetch masters first
    setLoadingCourses(true);
    const pCourses = API.get("/api/master/courses")
      .then((res) => {
    ...
    const pBranches = API.get("/api/master/branches")
      .then((res) => {
        const rawData = res.data;
        const data = Array.isArray(rawData) ? rawData : (rawData && Array.isArray(rawData.data) ? rawData.data : []);
    ...
    const pSubjects = API.get("/api/master/subjects")
      .then((res) => {
        const rawData = res.data;
        const data = Array.isArray(rawData) ? rawData : (rawData && Array.isArray(rawData.data) ? rawData.data : []);
    ...
    const pMaterialTypes = API.get("/api/master/material-types")
      .then((res) => {
        const rawData = res.data;
        const data = Array.isArray(rawData) ? rawData : (rawData && Array.isArray(rawData.data) ? rawData.data : []);
    ...
    // 2. Fetch record data and match
    Promise.all([pCourses, pBranches, pSubjects, pMaterialTypes]).then(() => {
      API.get(`/api/material-meta-1/${targetId}`)
        .then((res) => {
          if (res.data && typeof res.data === "object") {
            const r = res.data;
            setCourseId(String(r.courseId ?? r.course_id ?? ""));
            setBranchId(String(r.branchId ?? r.branch_id ?? ""));
            setSubjectId(String(r.subjectId ?? r.subject_id ?? ""));
            setSubjectShortForm(String(r.subjectShortForm ?? r.subject_short_form ?? ""));
            setMaterialTypeId(String(r.materialTypeId ?? r.material_type_id ?? ""));
            setYear(String(r.year ?? ""));
            setSemester(String(r.semester ?? ""));
            setChaptersCount(String(r.chaptersCount ?? ""));
            setDescription(String(r.description ?? ""));
            setStatus(r.status === undefined || r.status === true || r.status === "Active" || r.status === 1 || String(r.status).toLowerCase() === "true");
            setLoading(false);
          } else {
  ...
  const loadRecordFromLocalStorage = () => {
    ...
    const existing = stored ? JSON.parse(stored) as MaterialMeta1[] : [];
    const found = existing.find((r) => r.id === targetId);
    if (found) {
      setCourseId(found.courseId);
      setBranchId(found.branchId);
      setSubjectId(found.subjectId);
      setSubjectShortForm(found.subjectShortForm);
      setMaterialTypeId(found.materialTypeId);
      setYear(found.year);
      ...
      router.push("/transaction/material-meta-1-list");
    }
    setLoading(false);
  };
  ...
  // Filter subjects by Course and Branch
  ...
  const handleUpdate = () => {
    ...
    const updatedRecord: MaterialMeta1 = {
      id: targetId,
      courseId,
    ...
    API.put(`/api/material-meta-1/${targetId}`, payload)
      .then(() => {
        setToast("✓ Material Meta updated successfully");
      })
      .catch(() => {
        setToast("✓ Material Meta updated locally (backend offline)");
      })
      .finally(() => {
        const nextList = currentList.map((r) => (r.id === targetId ? updatedRecord : r));
        ...
        <div className="edit-user-header">
          <h1>
            <span style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <Edit size={24} /> Modify Material Meta
            </span>
          </h1>
        </div>
        ...
        <form className="form-two-col" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="form-col">
            <div className="edit-user-row compact">
              <label htmlFor="id-input">Material Meta ID</label>
          ...
          <div className="form-col">
            ...
            <div className="edit-user-row compact">
              ...
              <div className="edit-user-field">
                ...
                >
                  <option value="true" style={{ background: "#1a1f2e", color: "#4ade80" }}>Yes</option>
                  <option value="false" style={{ background: "#1a1f2e", color: "#f87171" }}>No</option>
                </select>
              </div>
            </div>
          </div>
