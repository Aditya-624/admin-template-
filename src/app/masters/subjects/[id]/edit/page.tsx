...
export default function EditSubjectPage() {
  ...
  const [status, setStatus] = useState("Yes"); // Yes maps to true, No maps to false
  ...
  useEffect(() => {
    ...
    API.get(`/api/master/subjects/${subjectId}`)
      .then((res) => {
        const s = res.data;
        if (s) {
          setCourseId(String(s.courseId ?? s.course_id ?? ""));
          setBranchId(String(s.branchId ?? s.branch_id ?? ""));
          setYear(String(s.year ?? ""));
          setSemester(String(s.semester ?? ""));
          setSubjectName(String(s.name ?? s.subjectName ?? s.subject ?? ""));
          setShortForm(String(s.shortForm ?? s.short_form ?? ""));
          setChaptersCount(String(s.chaptersCount ?? s.chapters_count ?? "10"));
          setInternalMarks(String(s.internalMarks ?? s.internal_marks ?? ""));
          setInternalPassMark(String(s.internalPassMark ?? s.internal_pass_mark ?? ""));
          setExternalMarks(String(s.externalMarks ?? s.external_marks ?? ""));
          setExternalPassMark(String(s.externalPassMark ?? s.external_pass_mark ?? ""));
          setDescription(String(s.description ?? ""));
          setStatus(s.status === false ? "No" : "Yes");
        }
        setSubjectLoading(false);
      })
      .catch((err) => {
        console.warn("Backend fetch failed, searching locally:", err);
        const stored = localStorage.getItem(storageKey);
        const currentSubjects: Subject[] = stored ? JSON.parse(stored) : mockSubjects;
        const found = currentSubjects.find((s) => s.id === subjectId);
        if (found) {
          ...
          setShortForm(found.shortForm);
          setChaptersCount(String(found.chaptersCount));
          setInternalMarks(String(found.internalMarks));
          setInternalPassMark(String(found.internalPassMark));
          setExternalMarks(String(found.externalMarks));
          setExternalPassMark(String(found.externalPassMark));
          setDescription(found.description);
          setStatus(found.status ? "Yes" : "No");
        }
        setSubjectLoading(false);
      });
  }, [subjectId]);
  ...
  const validate = () => {
    ...
    // 3. Uniqueness check (Rule 2: within selected branch only, excluding self)
    if (branchId && subjectName.trim()) {
      ...
      if (isDuplicate) {
        newErrors.subjectName = "Name duplicate in branch";
      }
    }
  ...
  const handleUpdate = () => {
    ...
    // Call PUT/PATCH API if available
    const payload = {
    ...
    console.log("Updating subject payload to backend API:", payload);

    API.put(`/api/master/subjects/${subjectId}`, payload)
      .then((res) => {
        console.log("Successfully updated subject in backend DB:", res.data);
        setToast("✓ Subject updated successfully");
      })
      .catch((err) => {
        console.warn("Backend PUT /api/master/subjects failed, updated locally:", err);
        setToast("✓ Subject updated locally (backend offline)");
      })
      .finally(() => {
        // Save to local storage for UI consistency
        const stored = localStorage.getItem(storageKey);
        const currentSubjects: Subject[] = stored ? JSON.parse(stored) : mockSubjects;
        const updatedSubjects = currentSubjects.map((s) => (s.id === subjectId ? updatedSubject : s));
  ...
  if (subjectLoading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "400px", color: "rgba(255,255,255,0.7)" }}>
        ...
        <form className="form-three-col" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="form-col">
            <div className="edit-user-row compact">
              <label htmlFor="subject-id-input">Subject ID</label>
          ...
          <div className="edit-user-actions form-actions-row">
            ...
            <button type="button" className="edit-user-update" onClick={handleUpdate}>
              Update
            </button>
          </div>
        </form>
